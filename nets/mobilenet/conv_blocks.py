...
def expanded_conv(input_tensor,
  ...
  with tf.variable_scope(scope, default_name='expanded_conv') as s, \
    ...
    '''
    if se:
    	# net_se = global_avg(net)
    	channel_num = net_se.shape[1].value
    	net_se = slim.fully_connected(net_se,channel_num,scope='fc_1',activation_fn=tf.nn.relu,weights_regularizer=slim.l2_regularizer(1e-4))
    	net_se = slim.fully_connected(net_se,channel_num,scope='fc_2',activation_fn=tf.nn.sigmoid,weights_regularizer=slim.l2_regularizer(1e-4))
    	net_se = tf.reshape(net_se,[-1,1,1,channel_num])
    	net = net*net_se
    	net = tf.identity(net, name='se_output')
    '''
    if endpoints is not None:
    ...
    # insert se module here
    reduction = 4
    if se:
      # net_se = global_avg(net)
      net_se = tf.identity(net)
      channel_num = net_se.shape[3].value
      # net_se = slim.fully_connected(net_se,channel_num,scope='fc_1',activation_fn=tf.nn.relu,weights_regularizer=slim.l2_regularizer(1e-4))
      net_se = slim.conv2d(net_se, channel_num // reduction, [1,1], padding="VALID",scope='se_1', activation_fn=tf.nn.relu)
      # net_se = slim.fully_connected(net_se,channel_num,scope='fc_2',activation_fn=tf.nn.sigmoid,weights_regularizer=slim.l2_regularizer(1e-4))
      net_se = slim.conv2d(net_se, channel_num, [1,1], padding="VALID",scope='se_2', activation_fn=tf.nn.sigmoid)
      # net_se = tf.reshape(net_se,[-1,1,1,channel_num])
      net = net*net_se
      net = tf.identity(net, name='se_output')
    if callable(residual):  # custom residual
      ...
      print("Success")
      shortcut=slim.conv2d(input_tensor, net.get_shape().as_list()[3], [1,1], padding="VALID",scope='shortcut_Conv', activation_fn=None)
      net += shortcut
    return tf.identity(net, name='output')

def global_avg(x,keep_dims=False,stride=1,rate=1,use_explicit_padding=True):
